...
public enum Motions
{
    ...
    SigniZonePowerUp,
    CantUseSpellAndArts,    //このターン、アーツとスペルを使用できない。
    EffectLostTempSummon,
    ArcDistract,
    DownSummon,
}
...
//	private Texture showTexture;
    ...
    bool encoreSelectflag = false;
    bool encoreAnswear = false;
    bool encoreReaded = false;
...
//	int[] LevelLimit = new int[2]{0,0};
    ...
    class EffTempSet
    {
        public int receiver = -1;
        public int giver = -1;
        public EffectTemplete temp;

        public EffTempSet(int receiverID, int giverID, EffectTemplete e)
        {
            receiver = receiverID;
            giver = giverID;
            temp=e;
    ...
    List<EffTempSet> enEffTempList = new List<EffTempSet>();
	...
	int tellEffectID=-1;
    int[] ViolenceCount = new int[2] { 0, 0 };
...
//	bool canReplaySend=true;
    ...
    int lastGoTrashCharmID = -1;

    bool[] cantUseArtsFlag = new bool[2];
    bool[] cantUseSpellFlag = new bool[2];
...
//	bool[] ionaFlag = new bool[2]{false,false};
    ...
    public bool isUsedThis(int ID, int player)
    {
        ...
        {
            if (checkName(ID, player, getCardScr(usedList[i]).Name))
                return true;
        }
    ...
    public bool isNotUsedOtherArts(string s)
    {
        for (int i = 0; i < usedList.Count; i++)
        {
            if (!checkName(usedList[i], s) && checkType(usedList[i], cardTypeInfo.アーツ))
                return false;
        }
...
//        return getCardScr(EffecterNowID).effectTargetID.Count == 0;*/
    ...
    public int getGoTrashCharmID()
    {
        if (getExitID(-1, -1)== -1 || getExitID(-1, -1) != lastGoTrashCharmID)
            return -1;

        return lastGoTrashCharmID;
    }

    public int getEffectExitID(int from, int to)
    {
        if (EffecterNowID == -1)
            return -1;

        return getExitID(from, to);
    }
    ...
    //カード情報のルリグタイプをチェックする
    public bool checkLrigTypeInfo(int x, int target, LrigTypeInfo info)
    {
        if (!checkType(x, target, cardTypeInfo.ルリグ))
            return false;
...
//        return (int)info == sc.LrigType || (int)info == sc.LrigType_2;
        return checkLrigTypeInfo(getLrigID(player), player, info);
    }
...
//			num += sc.Cost [i];
	...
	{
		if (ID < 0 || checkType(ID,player, cardTypeInfo.アーツ) || checkType(ID,player, cardTypeInfo.スペル))
			return -1;
    ...
    public bool checkLevelLower(int x, int target, int level)
    {
        int l = getCardLevel(x, target);
        if (l == -1)
            return false;
    ...
    {
        return checkAbility(x + 50 * target, a);
    }
    public bool checkAbility(int fusionID, ability a)
    {
        CardScript sc=getCardScr(fusionID);
        if(sc==null)
            return false;
        return sc.checkAbility(a);
    }
...
//        return sc.Name == key;
    ...
    public bool checkIsUp(int x, int target)
    {
        if (x < 0)
            return false;

        return getCardScr(x, target).isUp();
    }
...
/*        //violence
        ...
        //フィア＝ヴィックス
        if (AlwysEffFlags.checkFlagUp(alwysEffs.FiaVix, target) && target == player && up > 0)
            up *= -1;*/

        up = calcUpPower(x + 50 * target, ID + 50 * player, up);
...
/*		if (ID < 0)
    ...
    public bool checkCost2nd(int player, List<int> costColorList, List<int> payedCardList,bool isMelhenGrow)
    {
        ...
        return getPayableList(player, costColorList, payedCardList, isMelhenGrow).Count > 0;
/*
 ...
 //       bool flag = false;
        for (int i = 0; i < num; i++)
        {
            ...
            //コサキによる複数エナ対応
            int kosakiCount = nextGreenNum(subCostColorList);
            kosakiListRemove(color, fID, kosakiCount, subCostColorList);
            ...
            if (!payedCardList.Contains(fID) && checkCost2nd(player, subCostColorList, subPayedCardList,isMelhenGrow))
            {
                ...
                //コサキによる複数エナ対応
//                kosakiListRemove(color, fID, kosakiCount, costColorList);
//                flag = true;
//                break;
            }
        }
...
//        return checkCost2nd(player, costColorList, payedCardList, isMelhenGrow);*/
    ...
    List<int> getPayableList(int player, List<int> costColorList, List<int> payedCardList, bool isMelhenGrow)
    {
        List<int> payableList = new List<int>();
    ...
    {
        CardScript sc = getCardScr(fusionID);
        int target=fusionID/50;
        if (info == cardColorInfo.無色
            || sc.MultiEnaFlag
            || (info == cardColorInfo.白 && checkClass(fusionID, cardClassInfo.精像_美巧) && (melhenFlag[fusionID/50] || isMelhenGrow))
            || (info == cardColorInfo.白 && sc.WhiteEnaFlag)
            || checkPayingOsaki(info, fusionID)
            || sc.checkAbility(ability.LrigColorEna) && checkColor(getLrigID(target), target,info)
            )
            return true;
    ...
    bool checkPayingOsaki(cardColorInfo info, int fusionID)
    {
        return info == cardColorInfo.緑 && checkNameContainsOsaki(fusionID) && AlwysEffFlags.checkFlagUp(alwysEffs.Kosaki, fusionID/50);
    }

    int nextGreenNum(List<int> enaColr)
    {
        int count=0;
        for (int i = 0; i < enaColr.Count; i++)
        {
            if(enaColr[enaColr.Count - 1 - i] == (int)cardColorInfo.緑)
                count++;
        }
        return count;
    }

    void kosakiListRemove(cardColorInfo color, int fID, int kosakiCount, List<int> list)
    {
        if (checkPayingOsaki(color, fID))
            for (int k = 0; k < kosakiCount && k < 2; k++)
                list.RemoveAt(list.Count - 1);
    }



    bool checkNameContainsOsaki(int fid)
    {
        return checkContainsName(fid % 50, fid / 50, "オサキ");
    }
...
*/		
       ...
       cardStatusBufSet();
		...
		//ダイアログのチェック
        if (!DialogFlag && !SystemEffectFlag && !isUpEffect(getCardScr(DialogAnswearID)))
        {
            for (int i = 0; i < 100 + NewCardList.Count; i++)
            {
                ...
                if ((!effectFlag || i == effecter[0] || sc.ReplaceFlag) && isUpDialog(sc))
                {
                    ...
                    DialogAnswearID = DialogID;
                    DialogScript = sc;
                    DialogStr = sc.DialogStr;
                    DialogNum = sc.DialogNum;
                    DialogNumType _dtype = (DialogNumType)DialogNum;

                    //countMax
                    if (_dtype == DialogNumType.color)//色
                        DialogCountMax = 5;
                    else if (_dtype == DialogNumType.Level || _dtype == DialogNumType.Type)//DialogNum == 6)//レベル
                        DialogCountMax = 4;
                    else if (_dtype == DialogNumType.TopBottom || _dtype == DialogNumType.UpDown || _dtype == DialogNumType.playerSelect)//DialogNum == 4 || DialogNum == 5)//上下、アップダウン
                        DialogCountMax = 1;
                    else
...
//					}
    ...
    void nextPhaseButton()
    {
        ...
        if (!isTrans || getTurnPlayer() == 0)
        {
            if (/*GUI.Button(new Rect(buttonPos.x - w / 20, h - buttonPos.y, w / 10, w / 60), "") && Input.GetMouseButtonUp(0)
                ...
                && !selectSigniZoneFlag && !selectCardFlag && /*(!selectCursorFlag||phase==Phases.EnaPhase) && */!guardSelectflag && !SpellCutInSelectFlag && !encoreSelectflag
 ...
 //           GUI.Label(new Rect(buttonPos.x - w / 20 + w / 100, h - buttonPos.y, w / 10, w / 60), "NextPhase", NextStyle);
    ...
    void YesNoGUI()
    {
        bool flag = guardSelectflag || SpellCutInSelectFlag || selectAttackAtrs || encoreSelectflag;
...
//        if (GUI.Button(buttunRect1, YesStr))
        if(uiYes)
        {
            ...
            else if (encoreSelectflag)
            {
                encoreSelectflag = false;
                encoreAnswear = true;
            }
            else if (selectAttackAtrs)
            {
...
//        else if (DialogNum == 1 || DialogNum == 3 || DialogNum == 4 || DialogNum == 5 || DialogNum == 6 || DialogNum == 7 )
        else if (
            _dtype == DialogNumType.targetNum
            || _dtype == DialogNumType.color
            || _dtype == DialogNumType.TopBottom
            || _dtype == DialogNumType.UpDown
            || _dtype == DialogNumType.Level
            || _dtype == DialogNumType.Type
            || _dtype == DialogNumType.playerSelect
            )
        {
            GUI.Box(boxRect, "");
...
/*            switch (DialogNum)
            ...
            switch (_dtype)
            {
                case DialogNumType.color:
                    dstr = "宣言する色";
                    dstr += " -> " + (cardColorInfo)DialogCount;
                    break;

                case DialogNumType.TopBottom:
                    dstr = "上か下";
                    dstr += " -> " + (positionInfo)DialogCount;
                    break;

                case DialogNumType.UpDown:
                    dstr = "アップかダウン";
                    string s = "アップ";
                    if (DialogCount == 1)
                        s = "ダウン";
                ...
                case DialogNumType.Level:
                    dstr = "宣言するレベル";
                    dstr += " -> " + (DialogCount + 1);
                    break;
                case DialogNumType.Type:
                    dstr = "宣言するタイプ";
                    dstr += " -> " + (cardTypeInfo)DialogCount;
                    break;
                case DialogNumType.playerSelect:
                    dstr = "自分か相手";
                    string playerStr = "自分";
                    if (DialogCount == 1)
                        playerStr = "相手";
...
//			deckNum [player % 2]--;
	...
	{
        if (enajeFlag[player % 2] && toldBanishing)
            GoTrash(ID, player);
        else if (toldBanishing && checkType(ID, player%2, cardTypeInfo.レゾナ))
            GoLrigDeck(ID, player);
        else
        {
...
//		showCardBurstIcon=script.BurstIcon;*/
	...
	void setTargetCursorUp (int player)
	{
		...
		if (LrigCondition [player] == Conditions.Up 
		    ...
		    && (!AlwysEffFlags.checkFlagUp(alwysEffs.Iona,getTurnPlayer ())|| isAttackEnd ()))
...
//							showUpDate(targetCursorList[i].ID,targetCursorList[i].player);
			...
			if (selectEnaFlag)
			{
				if (targetCursorList.Count == 0 && enaColorList.Count > 0)
				{
                    ...
                    for (int k = 0; k < selectEnaNum; k++)
                        sub_color.Add(enaColorList[k]);

                    foreach (var _id in getPayableList(selectEnaPlayer, sub_color, clickCursorID, melhenFlag[selectEnaPlayer]))
                        setTargetCursorID(_id % 50, _id / 50);
...
/*                    for (int i = 0; i < fieldAllNum(Fields.ENAZONE, selectEnaPlayer); i++)
				...
				if (Input.GetMouseButtonDown (0))
				{
					for (int i=0; i<targetCursorList.Count; i++)
					{
						if (SelectedCard == targetCursorList [i].targetCursor)
						{
                            int fID=targetCursorList[i].ID + 50 * targetCursorList[i].player;
                            clickCursorID.Add(fID);

                            bool checkKosaki = checkPayingOsaki((cardColorInfo)enaColorList[selectEnaNum - 1], fID);
                            ...
                            if (checkKosaki)
                            {
                                ...
                                for (int k = 0; k < selectEnaNum; k++)
                                    sub_color.Add(enaColorList[k]);
                                for (int k = 0; k < nextGreenNum(sub_color) && k < 2; k++)
                                    selectEnaNum--;
                            }
							DestoryCursorAll ();
							break;
...
//					}
	...
	void MainPhaseMove (int player)
	{
            ...
            else if (Input.GetMouseButtonDown(0) && selectedID >= 0)
            {
                ...
                else if (selectPlayer == player)
                {
                    if (field[player, selectedID] == Fields.HAND && selectClickID == -1)
                    {
                        ...
                        else if (checkType(selectedID,player, cardTypeInfo.スペル) && !cantUseSpellFlag[player]) 
                            //card[player, selectedID].GetComponent<CardScript>().Type == 3)
                        {
                            selectClickID = selectedID;
                            motion = Motions.ChantSpell;
                            movePhase[player] = 0;
                        ...
                        selectedID == getLrigID(player))//(field[player, selectedID] == Fields.LRIGZONE) && fieldRank[player, selectedID] == lrigZoneNum[player] - 1)
                    {
		...
		else if (motion == Motions.ChantArts)
		{
			if (GUImoveID >= 0 &&
				...
				(checkType(GUImoveID, cardTypeInfo.アーツ) ///card [player, GUImoveID % 50].GetComponent<CardScript> ().Type == 1
				&& !card [player, GUImoveID % 50].GetComponent<CardScript> ().notMainArts) && !cantUseArtsFlag[player]))
			{
	...
	void AttackPhaseMove (int player)
	{
		...
		else if (motion == Motions.ChantArts)
		{
			if (GUImoveID >= 0)
			{
                ...
                if (sc.Type == 1 && !cantUseArtsFlag[GUImoveID / 50])
    ...
    void enAbilityListAdd(int receiver, int giver, ability a)
    {
        ...
        if (scr == null || (scr.checkAbility(a) && !checkContainsAbilitySet(receiver, a)))
            return;
    ...
    bool checkContainsAbilitySet(int receiver,  ability a)
    {
        for (int i = 0; i < enAbilityList.Count; i++)
        {
            if (enAbilityList[i].receiver == receiver && enAbilityList[i]._ability == a)
                return true;
        }
    ...
    void EndPhaseMove(int player)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            cantUseArtsFlag[i] = false;
            cantUseSpellFlag[i] = false;

            if (ViolenceCount[i] > 0)
            {
                ViolenceCount[i] = 0;
                requipFlag = true;
            }
        ...
        while (enEffTempList.Count > 0)
        {
            Destroy(enEffTempList[0].temp);
            enEffTempList.RemoveAt(0);
        }
	...
	void SigniAttack (int ID, int player)
	{
		...
		if (chainMotion [1] == 0)
		{
			...
			if (moveID [player] == -1 && rotaID [player] == -1)
			{
                ...
                if (getCardScr(AttackerID).checkAbility(ability.TopChargeAfterAttack))
                    for (int i = 0; i < getCardScr(AttackerID).getAbilityOtherCount(ability.TopChargeAfterAttack); i++)
                        getCardScr(AttackerID).setEffect(0, AttackerID / 50, Motions.TopEnaCharge);
...
//					chainMotion[1]=-1;
	...
	void attackAfterChoice (int attacked)
	{
		...
		if (chainMotion [1] == 2)
		{
			...
			if(!ImmortalFlag)
			{
                SetSystemCardFromCard(attackedID + attacked * 50, Motions.EnaCharge, BattleFinishID[1 - attacked], 1 - attacked);
				attackedID = -1;
...
/*                clickCursorID.RemoveAt(0);
        ...
        if (chainMotion[1] == 7 && !selectCursorFlag)
		{
			...
			if (moveID [attacked] == -1 && rotaID [attacked] == -1)
			{
                setOneFrameID(OneFrameIDType.DoubleCrashedID, clickCursorID[1]);
                endCrash();
/*				clickCursorID.Clear ();
				chainMotion [1] = -1;
    ...
    void endCrash()
    {
        ...
        {
            getCardScr(crasherID).setEffect(0, crasherID/50, Motions.TopEnaCharge);
            getCardScr(crasherID).setEffect(0, crasherID/50, Motions.TopEnaCharge);
        }
        if (checkAbility(crasherID % 50, crasherID / 50, ability.TwoBanishAfterCrash))
        {
            getCardScr(crasherID).setFuncEffect(-1, Motions.EnaCharge, 1 - crasherID / 50, Fields.SIGNIZONE, null);
            getCardScr(crasherID).setEffect(-1, 0, Motions.EnaCharge);
        }
        crasherID = -1;
...
/*            card[player % 2, ID].GetComponent<CardScript>().DoubleCrash = false;
            ...
            */
            ...
            for (int i = 0; i < enEffTempList.Count; )
            {
                var set = enEffTempList[i];
                if (set.receiver == getFusionID(ID, player % 2))
                {
                    Destroy(set.temp);
                    enEffTempList.RemoveAt(i);
                }
                else i++;
            }
            ...
            if (k >= 0)
            {
                lastGoTrashCharmID = k + player % 2 * 50;
		...
		else if (field [player % 2, ID] == Fields.ENAZONE)
		{
            ...
            //エナソート入れ込み
            SetSystemCard(player % 2, Motions.EnaSort);
		}
	...
	void selectCardListAttackArtsIn (int player)
	{
        ...
        addAttackArts(player, Fields.SIGNIZONE);
        addAttackArts(player, Fields.LRIGDECK);
        addAttackArts(player, Fields.TRASH);
 ...
 /*       for (int i = 0; i < 3; i++)
    ...
    void addAttackArts(int player, Fields _F)
    {
        int num = getNumForCard((int)_F, player);
        for (int i = 0; i < num; i++)
        {
            int x = fieldRankID(_F, i, player);
            if (_F == Fields.LRIGDECK && !checkType(x, player, cardTypeInfo.アーツ))
                continue;
 ...
 /*   int getIgniCostTargIndex(int x,int target)
    ...
    colorCostArry CreatEncoredCostArray(int ID, int player)
    {
        CardScript sc = getCardScr(ID, player % 2);
        return new colorCostArry(sc.Cost, sc.encoreCost);
    }


    bool canUseEncoreCost(int ID,int player)
    {
        CardScript sc = getCardScr(ID, player % 2);
        return !sc.encoreCost.isEmpty() && checkCost2nd(player, creatCostColorList(ID, player, CreatEncoredCostArray(ID,player)), null, false);
    }
	...
	void PayModeCost (int ID, int player, bool isGrow)
	{
        ...
        if (waitYou(player % 2) && clickCursorID.Count == 0)
		{
			...
			if (canRead ())
			{
                if (canUseEncoreCost(ID, player) && !encoreReaded)
                {
                    encoreReaded = true;
                    if (readMessage() == YesStr)
                        encoreAnswear = true;
                }
		...
		else
		{
            if (movePhase[player] == 0 && moveID[player] == -1)
            {
                ...
                if (canUseEncoreCost(ID,player))
                {
                    encoreSelectflag = true;
                    return;
                }
            }

            if (movePhase[player] == 0 && moveID[player] == -3)
            {
                if (clickCursorID.Count == 0)
                {
                    moveID[player] = -1;
                    ...
                    colorCostArry cost;
                    if (isGrow)
                        cost = copyCostArray(sc.GrowCost);
                    else if (encoreAnswear)
                        cost = CreatEncoredCostArray(ID, player % 2);
                    else
                        cost = copyCostArray(sc.Cost);
        ...
        {
            encoreAnswear = false;
            var effs = getCardEffects(ID, player % 2);
            var temp = sc.AddEffectTemplete(EffectTemplete.triggerType.Chant, effs.getEffect(CardEffectType.GoLrigDeckThisCard));
            temp.setAfterUseDestroy();
        }
...
/*		if(moveID[player] == -1 && rotaID[player]==-1){
    ...
    void effectFlagSetting(CardScript sc, int i)
    {
	...
	{
        if (DialogAnswearID != -1)
        {
            effectFlagSetting(getCardScr(DialogAnswearID), DialogAnswearID);
            DialogAnswearID = -1;
        }
		...
		for (int n=0; n<100; n++)
		{
            ...
            if (isUpEffect(sc) && effectFlag && i != effecter[0])
            {
                dualEffectTriggerd = true;
                return;
            }

            effectFlagSetting(sc, i);//まとめた
            /*
			if(isUpEffect(sc) && !effectFlag)
			{
				effectFlag = true;
    ...
    {
        if (!usedList.Contains(sc.ID + sc.player * 50))
            usedList.Add(sc.ID + sc.player * 50);
...
/*                    List<int> list = sc.Targetable;
	...
	bool ChoiceEffectBody(CardScript sc,int playerNum)
	{
        ...
        if (!isEffectBanishing(m) && isEffectRemoving(m) && !toldRemoving)
        {
            toldRemoving = true;
        ...
        else if (m == Motions.ArcDistract)
        {
            int fID = getFusionID(getLrigID(effectPlayer % 2), effectPlayer % 2);
/*            var obj = getFront(fID % 50, fID / 50);
            if (obj.GetComponent<CardEffects>() == null)
                obj.AddComponent<CardEffects>();
            var _cardEff = obj.GetComponent<CardEffects>();*/
            var _cardEff = getCardEffects(fID % 50, fID / 50);

            var eff = getCardScr(fID % 50, fID / 50).AddEffectTemplete(EffectTemplete.triggerType.attackThisTurnArcDistractOnly, true);
            eff.addEffect(_cardEff.getEffect(CardEffectType.GoLrigTrashLrigUnder), EffectTemplete.option.ifThen);
            eff.addEffect(_cardEff.getEffect(CardEffectType.GoLrigTrashLrigUnder), EffectTemplete.option.ifThen);
            eff.addEffect(_cardEff.getEffect(CardEffectType.UpThisCard));

            enEffTempList.Add(new EffTempSet(fID, EffecterNowID, eff));
        }
        else if (m == Motions.CantUseSpellAndArts)
        {
            cantUseArtsFlag[effectPlayer % 2] = true;
            cantUseSpellFlag[effectPlayer % 2] = true;
        }
        else if (m == Motions.SetDialog)
        {
            if (sc.messages.Count == 0)
            {
                sc.setDialogNum((DialogNumType)sc.getParameta(parametaKey.settingDialogNum), effectPlayer % 2);
                return false;
            }
        }
        ...
        else if (isEffectBanishing(m))
        {
            ...
            //ラナジェによるフラグアップ
            bool lanajeUp = false;
            if (!enajeFlag[effectPlayer % 2] && checkAbility(client, ability.Lanaje))
            {
                lanajeUp = true;
                enajeFlag[effectPlayer % 2] = true;
            }
        ...
        else if (m == Motions.EffectLostTempSummon)
        {
            sc.effectTargetID.Insert(1, sc.effectTargetID[0]);
            sc.effectMotion.Insert(1, (int)Motions.LostEffectEnd);

            sc.effectTargetID.Insert(2, sc.effectTargetID[0]);
            sc.effectMotion.Insert(2, (int)Motions.RebornEndPhase);
        }
            ...
            )
        {
            if (fieldAllNum(Fields.SIGNIZONE, effectPlayer % 2) < signiSumLimit[effectPlayer % 2])
            {
                ...
                else
                {
                    ...
                    //本体
                    if (m == Motions.DownSummon ||(m == Motions.DownSummonFromTrash && field[effectPlayer % 2, effectID] == Fields.TRASH))
    ...
    bool CheckEffectResist(int ID, int player, int effecterID, int effecterPlayer)
    {
        ...
        return checkRian_eff(ID, player, effecterID, effecterPlayer)
            ...
            || checkResiSigniEff(ID, player, effecterID, effecterPlayer)
            || checkResiNotArts(ID, player, effecterID, effecterPlayer)
            || checkResiYourWhiteCardEff(ID, player, effecterID, effecterPlayer)
            || checShubNiggurath(ID, player, effecterID, effecterPlayer)
    ...
    bool checShubNiggurath(int ID, int player, int effecter, int effectPlayer)
    {
        CardScript sc = getCardScr(ID, player);
        return AlwysEffFlags.checkFlagUp(alwysEffs.ShubNiggurath, player)
            && sc.isDown()
            && player != effectPlayer
            && checkType(effecter, effectPlayer, cardTypeInfo.シグニ)
            ;
    }

    bool typeToResiFlag(CardScript sc, cardTypeInfo info)
    {
        switch (info)
        {
            case cardTypeInfo.ルリグ:
                return sc.checkAbility(ability.resiLrigEffect);
            case cardTypeInfo.シグニ:
                return sc.checkAbility(ability.resiSigniEffect);
            case cardTypeInfo.アーツ:
                return sc.checkAbility(ability.resiArts);
        }

        return false;
    }

    bool checkResiTypeEff(int ID, int player, int effecter, int effectPlayer, cardTypeInfo info)
    {
        CardScript sc = getCardScr(ID, player);
        return player != effectPlayer &&  checkType(effecter, effectPlayer,info) && typeToResiFlag(sc,info);
    }
    bool checkResiLrigEff(int ID, int player, int effecter, int effectPlayer)
    {
        return checkResiTypeEff(ID, player, effecter, effectPlayer, cardTypeInfo.ルリグ);
    }
    bool checkResiSigniEff(int ID, int player, int effecter, int effectPlayer)
    {
        return checkResiTypeEff(ID, player, effecter, effectPlayer, cardTypeInfo.シグニ);
    }
    ...
    bool checkResiNotArts(int ID, int player, int effecter, int effectPlayer)
    {
        CardScript sc = getCardScr(ID, player);
        return player != effectPlayer && sc.checkAbility(ability.resiNotArts) && !checkType(effecter, effectPlayer, cardTypeInfo.アーツ);
    }
...
//		Shuffle (0);
	...
	string readMessage ()
	{
        ...
        if (!replayMode)
        {
            ...
            //このタイミングでリプレイを書き出し
            if (replayPath == "")
                replayPath = CreateReplayDirectory();
            saveReplay(replayPath);
        }
...
/*		for (int i=0; i<50; i++)
	...
	void afterPlayerDisconnected(){
		...
		{
			saveReplay(CreateReplayDirectory());
...
//		LevelLimit [player] = sc.Limit;		
	...
	void powerUpEndPhase (int ID, int player, int upValue, int effecter)
	{
        ...
        upValue = calcUpPower(ID + player % 2 * 50, effecter, upValue);

		SigniPowerUpValue [player, fieldRank [player, ID]] += upValue;
		upCardPower (ID, player, upValue);
	}

    int calcUpPower(int targetID, int effecter, int up)
    {
        //violence
        if(checkType(effecter, cardTypeInfo.シグニ))
            for (int i = 0; i < ViolenceCount[effecter / 50] && up < 0; i++)
                up *= 2;

        //フィア＝ヴィックス
        if (AlwysEffFlags.checkFlagUp(alwysEffs.FiaVix, targetID / 50) && targetID / 50 == effecter / 50 && up > 0)
            up *= -1;
...
/*						for (int i = 0; i < deckNum[1]; i++) {
	...
	{
		if (phase == Phases.AttackPhase
            && AlwysEffFlags.checkFlagUp(alwysEffs.Iona,getTurnPlayer())
//            && ionaFlag[getTurnPlayer()] 
            && isSigniAttackable ())
			return false;
...
//			replaySaveFlag = false;
    ...
    public CardEffects getCardEffects(int ID, int player)
    {
        int fID = getFusionID(ID, player%2);
        var obj = getFront(fID % 50, fID / 50);
        if (obj.GetComponent<CardEffects>() == null)
            obj.AddComponent<CardEffects>();
        var _cardEff = obj.GetComponent<CardEffects>();
