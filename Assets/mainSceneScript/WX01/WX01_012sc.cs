...
public class WX01_012sc : MonoBehaviour {
	GameObject Manager;
	...
	void Update () {
		...
		if(ID==ManagerScript.getFieldRankID(4,lrigDeckNum-1,player)){
			...
			for(int i=0;i<3;i++){
				...
				if(x[i]>=0){
					if(ManagerScript.getCardColor(x[i],player)==2)flag_r=true;
				}
			}
			if(flag_r){
				...
				for(int j=0;j<3;j++){
					if(!checkExist(x[j]) && x[j]>=0 && ManagerScript.getCardColor(x[j],player)==2){
						upList.Add(x[j]);
