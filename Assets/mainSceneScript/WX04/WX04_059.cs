...
public class WX04_059 : MonoBehaviour {
	GameObject Manager;
        ...
        if (ManagerScript.getFieldInt(ID, player) == 3)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                if (x >= 0 && !ManagerScript.checkChanListExist(x, target, ID, player))
                {
                    ...
                    if (ManagerScript.getCardColor(x, target) == targetColor)
	...
	bool upCondition(){
		...
		for(int i=0;i<3;i++){
			...
			if(x>=0 && ManagerScript.getCardColor(x,target)==targetColor)return true;
		}
