...
public class WD07_001 : MonoBehaviour {
	...
	void Update () {
		...
		if(ID==ManagerScript.getLrigID(player) && ManagerScript.isColorSigni(1,player) && ManagerScript.isColorSigni(5,player)){
			...
			for(int i=0;i<3;i++){
				x[i]=ManagerScript.getFieldRankID(3,i,target);
			}
			
			//signi check
/*			for(int i=0;i<upList.Count;i++)
			{
				if(ManagerScript.getFieldInt(upList[i],target)!=3)
				{
					ManagerScript.upCardPower(upList[i],target,-BodyScript.powerUpValue,ID+50*player);
					upList.RemoveAt(i);
					i--;
				}
			}*/
			...
			for(int j = 0; j < 3; j++)
			{
				if( x[j]>=0 && !checkExist(x[j], target)){
					
					//requirement add upList
					if(true){//all signi
						ManagerScript.alwaysChagePower(x[j], target, BodyScript.powerUpValue, ID, player);
					}
				}
			}
		}
	...
	bool checkExist(int x, int target){
		int[] array = ManagerScript.getChangedArray(ID,player);
