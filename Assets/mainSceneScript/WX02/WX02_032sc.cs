...
public class WX02_032sc : MonoBehaviour {
	GameObject Manager;
	...
	void Update () {
		...
		if(ManagerScript.getFieldInt(ID,player)==3 && field!=3 && !BodyScript.BurstFlag){
			...
			for(int i=0;i<3;i++){
				...
				if(x>=0 && ManagerScript.getCardPower(x,target)<=8000)flag=true;
			}
...
//				DialogFlag=true;
		...
		if(costFlag && BodyScript.effectTargetID.Count==0){
			...
			for(int i=0;i<3;i++){
				int x=ManagerScript.getFieldRankID(3,i,target);
				if(x>=0 && ManagerScript.getCardPower(x,target)<=8000)BodyScript.Targetable.Add(x+50*target);
			}
